...
(function () {
		...
		clone: function (G) {
			var E = this.map(function () {
				if (!o.support.noCloneEvent && !o.isXMLDoc(this)) {
					...
					return o.clean([
						I.replace(/ jQuery\d+="(?:\d+|null)"/g, "").replace(
							/^\s*/,
							""
						),
					])[0];
		...
		val: function (K) {
			if (K === g) {
				...
				if (E) {
					if (o.nodeName(E, "option")) {
						return (E.attributes.value || {}).specified
							? E.value
							: E.text;
					}
			...
			return this.each(function () {
				...
				if (o.isArray(K) && /radio|checkbox/.test(this.type)) {
					this.checked =
						o.inArray(this.value, K) >= 0 ||
						o.inArray(this.name, K) >= 0;
				} else {
					if (o.nodeName(this, "select")) {
						var N = o.makeArray(K);
						o("option", this).each(function () {
							this.selected =
								o.inArray(this.value, N) >= 0 ||
								o.inArray(this.text, N) >= 0;
						});
						if (!N.length) {
			...
			function K(N, O) {
				return M && o.nodeName(N, "table") && o.nodeName(O, "tr")
					? N.getElementsByTagName("tbody")[0] ||
							N.appendChild(
								N.ownerDocument.createElement("tbody")
							)
					: N;
			}
		},
	...
	o.extend({
		...
		className: {
			...
			remove: function (E, F) {
				if (E.nodeType == 1) {
					E.className =
						F !== g
							? o
									.grep(E.className.split(/\s+/), function (
										G
									) {
			...
			has: function (F, E) {
				return (
					F &&
					o.inArray(E, (F.className || F).toString().split(/\s+/)) >
						-1
				);
			},
		},
				...
				function I() {
					...
					o.each(K, function () {
						if (!E) {
							L -=
								parseFloat(
									o.curCSS(H, "padding" + this, true)
								) || 0;
						}
						if (E === "margin") {
							L +=
								parseFloat(
									o.curCSS(H, "margin" + this, true)
								) || 0;
						} else {
							L -=
								parseFloat(
									o.curCSS(H, "border" + this + "Width", true)
								) || 0;
						}
					});
				}
			...
			if (I) {
				for (var J = 0; G[J]; J++) {
					if (
						...
						(!G[J].type ||
							G[J].type.toLowerCase() === "text/javascript")
					) {
						E.push(
							G[J].parentNode
			...
			filter: {
				PSEUDO: function (Z, V, W, aa) {
					...
					if (X) {
						...
						if (U === "contains") {
							return (
								(Z.textContent || Z.innerText || "").indexOf(
									V[3]
								) >= 0
							);
						} else {
	...
	o.event = {
		...
		fix: function (H) {
			...
			if (!H.which && H.button) {
				H.which =
					H.button & 1 ? 1 : H.button & 2 ? 3 : H.button & 4 ? 2 : 0;
			}
			return H;
		},
		proxy: function (F, E) {
	...
	o.each({ mouseover: "mouseenter", mouseout: "mouseleave" }, function (
		F,
		E
	) {
		o.event.special[E] = {
	...
	(function () {
		...
		o.support = {
			...
			objectAll: !!K.getElementsByTagName(
				"object"
	...
	o.extend({
		...
		param: function (E) {
			...
			function H(I, J) {
				G[G.length] =
					encodeURIComponent(I) + "=" + encodeURIComponent(J);
			}
	...
	o.each(["Height", "Width"], function (I, G) {
		...
		o.fn["outer" + G] = function (K) {
			return this[0]
				? o.css(this[0], F, false, K ? "margin" : "border")
				: null;
		};
		var J = G.toLowerCase();
		o.fn[J] = function (K) {
...
(function ($) {
	...
	$.modal.impl = {
		...
		fixIE: function () {
			var s = this,
				p = s.o.position;
			$.each([s.d.iframe || null, s.d.overlay, s.d.container], function (
				i,
				el
			) {
				if (el) {
		...
		setContainerDimensions: function (resize) {
			...
			if (!resize || (resize && s.o.autoResize)) {
				...
				var mh =
						s.o.maxHeight && s.o.maxHeight < w[0]
							? s.o.maxHeight
							: w[0],
					mw =
						s.o.maxWidth && s.o.maxWidth < w[1]
							? s.o.maxWidth
							: w[1];
				if (!ch) {
					if (!dh) {
		...
		setPosition: function () {
			...
			if (
				s.o.position &&
				Object.prototype.toString.call(s.o.position) ===
					"[object Array]"
			) {
		...
		watchTab: function (e) {
			...
			if ($(e.target).parents(".simplemodal-container").length > 0) {
				...
				if (
					(!e.shiftKey &&
						e.target == s.inputs[s.inputs.length - 1]) ||
...
game.createScoreboard = function () {
	...
	for (var i = 1; i <= game.team_cnt; i++) {
		content +=
			"<th><input class='team-name' type='text' value='Team " +
			i +
			"' /></th>";
	}
	content += "</tr><tr>";
...
prompt.show = function (questionID, points) {
	...
	if ($("#question").html().length == 0) $("#correct-response").hide();
	else $("#correct-response").show();
};
